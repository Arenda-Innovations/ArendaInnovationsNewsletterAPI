...
function sendEmailConfirmation(req: any, userId: string ) {
 const sgMail = require('@sendgrid/mail')
  sgMail.setApiKey(process.env.SENDGRID_API_KEY)
  ...
  const msg = {
    ...
    subject: 'Sending with SendGrid is Fun',
    text: 'and easy to do anywhere, even with Node.js',
    html: `Hello to confirm your email go to this link: <a href="${process.env.FRONTEND_URL}/confirm-email/${userId}">Confirm Email</a>`,
  }
  sgMail
    .send(msg) 
    .then(() => {
      return ('Email sent' + req.body);
    })
    .catch((error: any) => {
...
app.put('/', (req, res) => {
  if (req.body.email != null) {
    ...
    const response = sendEmailConfirmation(req, userId);
    //Append to CSV file
    const data = `${userId}, ${req.body.email}\n`;
    fs.appendFile('src/Needconfirmation.csv', data)
      .then(() => {
